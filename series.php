...
// Si hay búsqueda, obtener los resultados
if (!empty($search_query)) {
    try {
        $media_results = buscarMedia($conn, $search_query, $limit, $offset);
...
<body class="antialiased">
    ...
    <div class="pt-20"></div> <!-- Espacio para que el contenido no quede bajo el nav -->
                    ...
                    <?php foreach ($media_results as $item): ?>
                        <?php
                            ...
                            $url = ($item['media_type'] === 'movie') 
                                ? "get_movie.php?type=movie&tmdb_id=" . urlencode($item['tmdb_id']) 
                                : "get_seasons.php?type=series&tmdb_id=" . urlencode($item['tmdb_id']);
                        ?>
                        <div class="card relative group rounded-lg overflow-hidden shadow-lg">
                            <img src="<?php echo htmlspecialchars($poster, ENT_QUOTES, 'UTF-8'); ?>" alt="<?php echo htmlspecialchars($item['titulo'] ?? 'Sin título', ENT_QUOTES, 'UTF-8'); ?>" class="w-full h-80 object-cover">
                            <div class="absolute inset-0 bg-gray-900 bg-opacity-75 text-white flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 p-4 text-center">
                                <h3 class="text-lg font-bold mb-2"><?php echo htmlspecialchars($item['titulo'] ?? 'Sin título', ENT_QUOTES, 'UTF-8'); ?></h3>
                                <p class="text-sm mb-4"><?php echo htmlspecialchars($item['release_date'] ?? '', ENT_QUOTES, 'UTF-8'); ?></p>
                                <a href="<?php echo $url; ?>" class="block mt-4 text-white hover:text-gray-300">
                                    <button 
    ...
    <?php else: ?>
        ...
        <!-- Si no hay búsqueda, mostrar el slider y las series populares -->
       
            <!-- Slider de Contenido Reciente -->

         <!-- Si no hay búsqueda, mostrar el slider y las peliculas populares -->
                        ...
                        <div class="swiper-slide">
                            <div class="movie-card flex items-center">
                                ...
                                <div class="movie-info p-4">
                                    ...
                                    <!-- Si desea mostrar la descripción, se puede hacer así -->
        ...
        <div class="container mx-auto px-6 py-8">
            <!-- Sección de series -->
            <section class="mb-12">
                <h2 class="text-3xl font-bold mb-4">Series Populares</h2>
                <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    <?php foreach ($web_series as $serie): ?>
                        <div class="card relative group rounded-lg overflow-hidden shadow-lg">
                            <img src="<?php echo htmlspecialchars($serie['poster'], ENT_QUOTES, 'UTF-8'); ?>"
                                alt="<?php echo htmlspecialchars($serie['titulo'], ENT_QUOTES, 'UTF-8'); ?>" class="w-full h-80 object-cover">
                            <div class="absolute inset-0 bg-gray-900 bg-opacity-75 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                <a href="get_seasons.php?type=series&tmdb_id=<?php echo htmlspecialchars($serie['tmdb_id'], ENT_QUOTES, 'UTF-8'); ?>" class="block mt-4 text-white-1900 hover:text-white-1300 ">
                                    <button 
                                        class="play-button w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg transition-colors flex items-center justify-center gap-2"
                                    >
                                        <i class="fas fa-play"></i>
                            ...
                            <p class="text-gray-400 text-center mt-2"><?php echo htmlspecialchars($serie['release_date'], ENT_QUOTES, 'UTF-8'); ?></p>
                            <div class="flex justify-center space-x-1 text-yellow-400 mt-1">
                                <?php
                                $user_score = $serie['user_score'] ?? 0;
                                for ($i = 1; $i <= 5; $i++): ?>
                            ...
                            <div class="prose prose-invert max-w-none">
                                <p class="text-gray-300 text-center leading-relaxed text-base md:text-lg">
                                    <?php echo htmlspecialchars($serie['titulo'], ENT_QUOTES, 'UTF-8'); ?>
                                </p>
                            </div>
                        </div>
                    <?php endforeach; ?>
                ...
                <div class="mt-4 flex justify-center space-x-4">
                    <?php if ($page > 1): ?>
                        <a href="?page_web_series=<?php echo $page - 1; ?>" class="px-4 py-2 bg-gray-700 text-gray-300 rounded-lg hover:bg-gray-600">Anterior</a>
                    <?php endif; ?>
                    <?php if ($page < $total_pages_series): ?>
                        <a href="?page_web_series=<?php echo $page + 1; ?>" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Siguiente</a>
                    <?php endif; ?>
                </div>
            </section>
        </div>
